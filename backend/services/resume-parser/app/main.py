...
async def upload_and_parse_resume(
    ...
    try:  # <--- ADD TRY BLOCK
        ...
        print(f"Error parsing file: {e}")  # print the error to console
        raise HTTPException(
            status_code=400,
            detail="Failed to parse the uploaded file. It might be corrupted."
            )
    ...
    if skills:
        pass
        # Prepare the data in the format required by the User API's
        # SkillCreate schema
        ...
        #         raise HTTPException(
        # status_code=503,
        # detail=f"Could not connect to User API: {e}"
        #         )
        #     except httpx.HTTPStatusError as e:
        #         # The User API returned an error (e.g., 401 Unauthorized)
        #         raise HTTPException(
        # status_code=e.response.status_code,
        # detail=f"Error from User API: {e.response.text}"
        #         )
